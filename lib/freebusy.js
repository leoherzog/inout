...
function mycalendar_checkBusy(cal) {
  ...
  reqEvents.execute(function(feed) {
    ...
    if (state == 2) { // busy
      ...
      // this block looks for long addresses with the city name (config.js) in it and chops it off
      ...
      if ( (typeof ev.summary !== 'undefined' && (ev.summary.toLowerCase().includes("vacation") || ev.summary.toLowerCase().includes("trip"))) ||
      (typeof ev.location !== 'undefined' && (ev.location.toLowerCase().includes("vacation") || ev.location.toLowerCase().includes("trip"))) ) {
